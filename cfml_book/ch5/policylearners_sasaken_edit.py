...
class GradientBasedPolicyLearner:
    ...
    def __post_init__(self) -> None:
        ...
        self.test_value = []
        self.pi_list = [] # 佐々木追記
        self.loss_list = [] # 佐々木追記
        self.grad_list = [] # 佐々木追記
    ...
    def fit(self, dataset: dict, dataset_test: dict, q_hat: np.ndarray = None) -> None:
        ...
        for _ in range(self.max_iter):
            ...
            for x_, a_, r_, p, q_hat_, pi_0_ in training_data_loader:
                ...
                optimizer.step()
                loss_epoch += loss.item()
                self.pi_list.append(pi) # 佐々木追記
                self.loss_list.append(loss) # 佐々木追記
                self.grad_list.append(self._estimate_policy_gradient(
                    a=a_,
                    r=r_,
                    pscore=p,
                    q_hat=q_hat_,
